...
def newton(
    ...
    while state_imp:
        n_iter += 1
        p = np.dot(np.linalg.inv(H(x_opt)), g(x_opt)).reshape(1, 2)
        x_opt = x_opt - p
        x_steps.append(x_opt)
        ...
        elif np.linalg.norm(g(x_opt)) < eps and np.all(np.linalg.eigvals(H(x0))):
...
x_opt, x_steps, fc, gc = gradient_descent(f, g, x0)
